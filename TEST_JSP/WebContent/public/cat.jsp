...
<div class="content_resize">
  <div class="mainbar">
  	...
  	%>
  	<%
  		if(request.getAttribute("danhmuc")!=null){
  			category dm =(category)request.getAttribute("danhmuc"); %>
  			<div class="article">
  			<h1><%=dm.getName()  %></h1>
  	    	</div>
  	<%	
    ...
    <div class="article">
  	...
  	if(request.getAttribute("listsong1")!= null){
			...
			for(Song song : listsong1){ %>
		      ...
		      <p class="infopost">Ngày đăng:<%=song.getDatecreate() %>. Lượt xem: <%=song.getCounter() %> <a href="#" class="com"><span><%=song.getId() %></span></a></p>
		      <div class="clr"></div>
   ...
   <%
  		if(request.getAttribute("sotrang")!= null){
  			...
  			if(request.getAttribute("idcate") != null){
  				iddm = (int)request.getAttribute("idcate");
  			}
			...
			if((float)trang/DefineUtil.NUMBER_PER_PAGE2 > 1){ %>
				<a href="<%=request.getContextPath()%>/Trang-chu?trang=<%=(trang-sd) %>&name=<%=name%>&idcate=<%=iddm%>"><<</a>
	<%		}
  			if(sotrang > 0){
  			%>
  			...
  			<%  for(int i = trang - (sd-1) ; i <= trang + (DefineUtil.NUMBER_PER_PAGE2-sd) ; i++){
  				...
  				<%		}
	  						...
	  						<a href="<%=request.getContextPath()%>/danh-muc?trang=<%=i %>&name=<%=name%>&idcate=<%=iddm%>"><%=i %></a>
	  			<%		}
  					}
  				}
	  			if((trang + DefineUtil.NUMBER_PER_PAGE2 - sd) < sotrang){ %>
	  				<a href="<%=request.getContextPath()%>/Trang-chu?trang=<%=(trang+DefineUtil.NUMBER_PER_PAGE2+1-sd) %>&name=<%=name%>&idcate=<%=iddm%>">>></a>	
		<%		}
  			}
  		}
  	%>
