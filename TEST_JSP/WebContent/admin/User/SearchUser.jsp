...
<div id="page-wrapper">
	<div id="page-inner">
		<div class="row">
			...
			<div class="col-md-12">
				<h2 style="color: blue;">
					<span>Không tồn tại người dùng cần tìm kiếm</span>
				</h2>
			</div>
			<%	}
	        	%>
								...
								<div class="col-sm-6" style="text-align: right;">
									<form method="get"
										...
										<input type="submit" name="search" value="Tìm kiếm" id = "sm-s"
											...
											type="search" id = "keyword" name="user"
											value="<%if(request.getAttribute("user")!=null){String user = (String)request.getAttribute("user");%> <%=user %> <% } %>"
                                        ...
                                        <%
                                        	if(request.getAttribute("sotrang") != null){
                                        		...
                                        		if((float)trang/DefineUtil.NUMBER_PER_PAGE1 > 1){ %>
                                        			<li class="paginate_button previous " aria-controls="dataTables-example" tabindex="0" id="dataTables-example_previous"><a href="<%=request.getContextPath()%>/Admin/search/user?trang=<%=trang-sd%>&user=<%=user %>">Trang trước</a></li>
                                        <%		}
                                        		if(sotrang > 0){
                                        			...
                                        			for(int i = trang - (sd-1) ; i <= trang + (DefineUtil.NUMBER_PER_PAGE2-sd) ; i++ ){
                                    					if(i > sotrang){
                                    						...
                                    						if(i == trang){ %>
                                                                <li class="paginate_button active" aria-controls="dataTables-example" tabindex="0"><a href="<%=request.getContextPath()%>/Admin/user/search?trang=<%=i%>&user=<%=user %>"><%=i %></a></li>
                                    <%						}
                                    						else{ %>
                                    							<li class="paginate_button" aria-controls="dataTables-example" tabindex="0"><a href="<%=request.getContextPath()%>/Admin/user/search?trang=<%=i%>&user=<%=user %>"><%=i %></a></li>
                                    <%							}
                                    					}
                                        			}
                                        			if((trang + DefineUtil.NUMBER_PER_PAGE1 - sd) < sotrang){ %>
                                        				<li class="paginate_button next" aria-controls="dataTables-example" tabindex="0" id="dataTables-example_next"><a href="<%=request.getContextPath()%>/Admin/user/search?trang=<%=(trang+DefineUtil.NUMBER_PER_PAGE1+1-sd)%>">Trang tiếp</a></li>		
                                        <%			}
                                        		}
                                        	}
                                        %>
